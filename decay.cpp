...
int main(){
  ...
  TH1F hist_smear_3("hist_smear","Measured invariant mass in LAB frame",n_bins,0,10);
  TH1F hist_smear_1("hist_smear","Measured invariant mass in LAB frame",n_bins,0,10);
  TH1F hist_smear_5("hist_smear","Measured invariant mass in LAB frame",n_bins,0,10);
  TH1F hist_smear_10("hist_smear","Measured invariant mass in LAB frame",n_bins,0,10);  
  ...
  for(double sig:sigma){
    for(int i=0;i<samples;i++){
      ...
      p_K_0=(p4_K.Vect()).Mag();
      p_pi_meas=gaus->TRandom::Gaus(p_pi_0,sig*p_pi_0);
      ...
      p_K_meas=gaus->TRandom::Gaus(p_K_0,sig*p_K_0);
    ...
    cout<<count<<"\t,"<<typeid(hist[count]).name()<<endl;
    hist_s.GetXaxis()->SetTitle("Invariant Mass [GeV]");
    hist_s.Draw();
    hist[count].Draw("same");
    ...
    cout<<p4_pi.Angle(p4_K.Vect())<<endl;
